...
def ingest(
    ...
    try:
        ...
        # Update metrics
        ingest_requests_total.labels(outcome="success").inc()
        ingest_records_total.labels(status="valid").inc(
            production_stats["valid_records"]
        )
        ingest_records_total.labels(status="invalid").inc(
            production_stats["invalid_records"]
        )
