...
class WildfireConsumer:
    ...
    def process_message(self, message: Any) -> bool:
        ...
        try:
            ...
            if success:
                logger.debug(f"Successfully processed message from partition {message.partition()}")
            else:
                logger.warning(f"Failed to process message from partition {message.partition()}")
    ...
    def consume_messages(self) -> None:
        ...
        try:
            ...
            logger.info(f"Consumer shutdown. Processed: {processed_count}, Errors: {error_count}")
