...
1. Send ingestion command to Kafka (hands work to the broker; ingestion runs asynchronously)
2. Optionally send map regeneration command (opt-in via param; otherwise maps follow processed events only)
...
dag = DAG(
    ...
    params={
        ...
        # Maps are regenerated by the map consumer from processed events; set True only
        # when you need an explicit regenerate_map command (decoupled from ingestion).
        "send_explicit_map_regeneration": False,
        "map_center_lat": 37.0,
...
def send_map_regeneration_command_to_kafka(**context) -> None:
    """
    Send a map regeneration command to Kafka (optional, opt-in).
    ...
    """
    params = context.get("params", {})
    if not params.get("send_explicit_map_regeneration", False):
        print(
            "Skipping explicit map regeneration command "
            "(send_explicit_map_regeneration=false; maps update from processed events)."
        )
        return
...
map_command_task = PythonOperator(
    ...
    execution_timeout=timedelta(minutes=10),
    doc_md="""
    Optionally send map regeneration command to Kafka (opt-in via DAG param
    ``send_explicit_map_regeneration``).
...
# Task dependencies: publish commands only (no wait for ingestion/map services).
# Ingestion and optional map commands are independent Kafka publishes — run in parallel
# so the scheduler does not imply a pipeline ordering beyond "commands were sent".
